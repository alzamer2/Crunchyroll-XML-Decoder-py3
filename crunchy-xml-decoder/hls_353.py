...
async def download_coroutine(session, url,output, key, media_sequence,output_offset, tasks):
    ...
    async with session.get(url) as response:
        ...
        with open(filename, 'ab') as f_handle:
            while True:
                ...
                output_offset.progress_bar_print[tasks.index(asyncio.Task.current_task())] = progress_bar_(output_offset.part_offset[str(media_sequence)],int(response.headers['Content-Length']),
...
async def compute_total_size_coroutine(session, output_offset, url, n):
...
async def compute_total_size(loop,video,tasks,output_offset):
    async with aiohttp.ClientSession(loop=loop) as session:
        for n in range(len(video)-1,-1,-1):
            finished_download_ = True
            for i in tasks[1:]:
                finished_download_ = finished_download_ and i.done()
            if finished_download_:
                tasks[0].cancel()
            if not str(video[n].media_sequence) in output_offset.part_size:
                await compute_total_size_coroutine(session, output_offset, video[n].absolute_uri, str(video[n].media_sequence))

    
 
async def main_download(loop,video,output,key,output_offset, tasks):
 
    async with aiohttp.ClientSession(loop=loop) as session:
        for n, seg in enumerate(video):
            await download_coroutine(session, seg.absolute_uri,output, key, seg.media_sequence,output_offset, tasks)

async def compute_total_size_tasks(loop,video,tasks,output_offset,connection_n):
    tasks_ = []
    for n, i in enumerate(range(0,len(video.segments),math.ceil(len(video.segments)/connection_n))):
        task_ = asyncio.ensure_future(compute_total_size(loop, video.segments[i:i+math.ceil(len(video.segments)/connection_n)],
                                                       tasks, output_offset))
        tasks_.append(task_)
        await asyncio.gather(*tasks_)

async def main(loop,video,output,connection_n):
    tasks = []
    ...
    tasks.append(asyncio.ensure_future(compute_total_size_tasks(loop,video,tasks,output_offset,connection_n)))
    for n, i in enumerate(range(0,len(video.segments),math.ceil(len(video.segments)/connection_n))):
        output_offset.progress_bar_print.append('')
        if hasattr(video, 'key'):
            task = asyncio.ensure_future(main_download(loop, video.segments[i:i+math.ceil(len(video.segments)/connection_n)],
...
def video_hls(uri, output, connection_n):
    ...
    future = asyncio.ensure_future(main(loop,video,output,connection_n))
